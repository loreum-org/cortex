...
func DefaultEmbeddedManagerConfig() EmbeddedManagerConfig {
	return EmbeddedManagerConfig{
		...
		AutoStart:    true,
		AutoModels:   []string{"llama2", "nomic-embed-text"},
		HealthCheck:  true,
	}
}
...
func (emm *EmbeddedModelManager) autoRegisterModels() {
	if len(emm.autoModels) == 0 {
		return
	}
	
	log.Printf("Auto-registering %d models...", len(emm.autoModels))
	
	for _, modelName := range emm.autoModels {
		if err := emm.RegisterOllamaModel(modelName, true); err != nil {
			log.Printf("Failed to auto-register model %s: %v", modelName, err)
		}
	}
...
func (emm *EmbeddedModelManager) updateDefaultModel() {
	...
	// Fallback to llama2 if cogito not found
	if defaultModelID == "" {
		for _, model := range models {
			if strings.Contains(model.ID, "llama2") {
				defaultModelID = model.ID
				break
			}
		}
