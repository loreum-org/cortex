...
const EconomicDashboard: React.FC = () => {
      ...
      const mockRewards: RewardEvent[] = [
        {
          id: 'reward_1',
          node_id: 'node-1',
          amount: '500000000000000000', // 0.5 λore
        ...
        },
        {
          id: 'reward_2',
          node_id: 'node-2',
          amount: '750000000000000000', // 0.75 λore
        ...
        },
        {
          id: 'reward_3',
          node_id: 'node-3',
          amount: '1200000000000000000', // 1.2 λore
        ...
        },
        {
          id: 'reward_4',
          node_id: 'node-1',
          amount: '600000000000000000', // 0.6 λore
  ...
  const performTransfer = async (e: React.FormEvent) => {
    ...
    try {
      // Convert λore amount to smallest unit
      ...
      if (response.ok) {
        setTransferForm(prev => ({
          ...prev,
          success: `Successfully transferred ${transferForm.amount} λore to ${transferForm.toAccount}`,
          amount: '',
          description: ''
        }));
  ...
  const performStake = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStakeForm(prev => ({
          ...prev,
          success: `Successfully staked ${stakeForm.amount} λore to node ${stakeForm.nodeId}`,
          amount: ''
        }));
  ...
  const formatTokenAmount = (amount: string) => {
    if (!amount) return '0 λore';
    
    // Convert from smallest unit (10^18) to λore
    const amountBigInt = BigInt(amount);
    const λoreAmount = Number(amountBigInt) / Math.pow(10, 18);
    
    return `${λoreAmount.toLocaleString(undefined, { maximumFractionDigits: 4 })} λore`;
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pricingTiers.map((tier) => (
            <div key={tier.id} className="bg-slate-700 border-2 border-slate-600 rounded-lg p-6 shadow-lg">
              ...
              <div className="text-lg font-semibold text-blue-400 mb-4">Base Price: {tier.basePrice} λore</div>
              ...
              <div className="border-t border-slate-600 pt-4">
                ...
                {Object.entries(tier.queryTypes).map(([type, price]) => (
                  <div key={type} className="flex justify-between py-1 text-sm text-slate-300">
                    ...
                    <span className="text-white font-medium">{price} λore</span>
                  </div>
                ))}
              </div>
            </div>
