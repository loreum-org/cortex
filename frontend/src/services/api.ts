...
class CortexAPI {
  ...
  constructor(baseURL?: string) {
    // Auto-detect the environment and set the appropriate base URL
    if (baseURL) {
      this.baseURL = baseURL;
    } else {
      // Check if we're in a Wails desktop environment
      const isDesktop = window.location.protocol === 'wails:' || 
                       window.location.hostname === 'wails.localhost' ||
                       (window as any).wails !== undefined;
      
      // In desktop mode, connect directly to cortexd server
      // In web mode, use relative URLs (handled by Vite proxy in dev or served together in prod)
      this.baseURL = isDesktop ? 'http://localhost:4891' : '';
    }
  }
  ...
  async getNetworkOverview(ports: number[] = [4891]): Promise<NetworkOverview> {
  ...
  async getNetworkTransactions(ports: number[] = [4891]): Promise<TransactionData[]> {
  ...
  async getAgents(): Promise<any[]> {
    const response = await this.request<{ agents: any[]; total: number }>('/agents');
  ...
  // Code Quality API methods
  async triggerCodeAnalysis(analysisType: string = 'full', targetPath: string = '.'): Promise<any> {
    return this.request<any>('/api/agents/code-quality/analyze', {
      method: 'POST',
      body: JSON.stringify({
  ...
  async getCodeQualityIssues(): Promise<any> {
    return this.request<any>('/api/agents/code-quality/issues');
  }

  async getCodeQualityMetrics(): Promise<any> {
    return this.request<any>('/api/agents/code-quality/metrics');
  }

  // Model Management API methods
  async getInstalledModels(): Promise<any[]> {
    return this.request<any[]>('/models/installed');
  }
  ...
  async downloadModel(modelName: string): Promise<any> {
    return this.request<any>('/models/download', {
      method: 'POST',
  ...
  async getDownloadProgress(taskId: string): Promise<any> {
    return this.request<any>(`/models/download/progress/${taskId}`);
  }

  async deleteModel(modelName: string): Promise<any> {
    return this.request<any>('/models/delete', {
      method: 'POST',
      body: JSON.stringify({ model_name: modelName }),
    });
  }
